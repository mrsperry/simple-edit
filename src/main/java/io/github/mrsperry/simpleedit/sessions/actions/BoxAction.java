...
public final class BoxAction extends Action {
    private BoxAction(final List<Block> blocks, final List<Pair<Material, Integer>> materials) {
    ...
    public static void run(final Selection selection, final boolean hollow, final List<Pair<Material, Integer>> materials) {
        new BoxAction(hollow ? selection.getEdgeSelection() : selection.getFaceSelection(), materials);
    }
}
